...
class Admin extends CI_Controller
{
    ...
    function receptionistShowAll(){
       $query= $this->crud_model->select_receptionist_info();
             if($query){
                   $result['receptionists']  = $this->crud_model->select_receptionist_info();
             }
        echo json_encode($result);
    }
    function receptionistAction($task = "", $receptionist_id = "") {
        if($this->session->userdata('admin_login') != 1) {
            $this->session->set_userdata('last_page', current_url());
            redirect(site_url(), 'refresh');
        }
